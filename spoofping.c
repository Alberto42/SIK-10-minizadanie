...
void send_spoof_ping_request(int sock, char *s_dst_addr, char *s_spoof_addr) {
    ...
    if (inet_pton(AF_INET, s_dst_addr,
                  (struct in_addr *) &dst_addr.sin_addr.s_addr) != 1)
        syserr("inet_pton (dst)");
    dst_addr.sin_family = AF_INET;
    dst_addr.sin_port = 0;

    memset(&spoof_addr, 0, sizeof(dst_addr));
    if (inet_pton(AF_INET, s_spoof_addr,
                  (struct in_addr *) &spoof_addr.sin_addr.s_addr) != 1)
    ...
    data_len = snprintf(
            ((char *) send_buffer + IP_HEADER_LEN + ICMP_HEADER_LEN),
            sizeof(send_buffer) - ICMP_HEADER_LEN - IP_HEADER_LEN,
            "BASIC PING!");
    if (data_len < 1)
        syserr("snprinf");
    ...
    len = sendto(sock, (void *) send_buffer, ntohs(ip->tot_len), 0,
